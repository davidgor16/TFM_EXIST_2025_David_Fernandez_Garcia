...
SEED = 42
BATCH_SIZE = 8  # Aumentado para textos, ya que suelen requerir menos memoria que audio
LEARNING_RATE = 2e-5  # Tasa de aprendizaje típica para modelos de texto
NUM_EPOCHS = 3
BETA = 0.5
MAX_LENGTH = 512  # Longitud máxima de secuencia para el tokenizador
...
# Dataset personalizado para textos
class TextClassificationDataset(Dataset):
    def __init__(self, texts, labels, ids, tokenizer):
        self.texts = texts
        self.labels = labels
        self.ids = ids
    ...
    def __getitem__(self, idx):
        ...
        # Extraer los tensores relevantes
...
# Modelo para clasificación binaria basado en modelos transformers para texto
class TextModelForBinaryClassification(nn.Module):
...
def load_data(json_path):
    ...
    return df['transcription'].values, df['label'].values, df["original_key"].values
...
def main():
    ...
    json_path = '../hghub/ownDatasets/TFM_EXIST/texto/train/metadata_EXIST2025_transcripcion_whisperX.json'
    
    # Cargamos los datos
    texts, ground_truth, ids = load_data(json_path)
    ...
    if stage == "AJUSTE_HIPERPARAMETROS":
        ...
        X_train, X_val, y_train, y_val, ids_train, ids_val = train_test_split(
            texts, ground_truth, ids, train_size=train_size, random_state=SEED
        )
        
        print(f"Datos de entrenamiento: {len(X_train)}, Datos de validación: {len(X_val)}")
        sys.stdout.flush()
        
        # Cargar el procesador de una vez
        processor = AutoTokenizer.from_pretrained(MODEL_NAME, local_files_only=False)
        ...
        def objective(trial):
            ...
            train_dataset = TextClassificationDataset(X_train, y_train, ids_train, processor)
            val_dataset = TextClassificationDataset(X_val, y_val, ids_val, processor)
            ...
            # Inicializar modelo
            model = TextModelForBinaryClassification(MODEL_NAME).to(DEVICE)
                        
            # Optimizador con tasas de aprendizaje diferentes
            optimizer = optim.AdamW([
                {'params': model.transformer.parameters(), 'lr': lr / 10},
        ...
        # Cargar el procesador  una vez (no cambia entre folds)
        processor = AutoTokenizer.from_pretrained(MODEL_NAME, local_files_only=False)
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(texts)):
            ...
            # Optimizador con tasas de aprendizaje diferentes
            optimizer = optim.AdamW([
                {'params': model.transformer.parameters(), 'lr': best_params['learning_rate'] / 10},
        ...
        full_train_dataset = TextClassificationDataset(texts, ground_truth, ids, processor)
        full_train_loader = DataLoader(full_train_dataset, batch_size=BATCH_SIZE, shuffle=True)

        # Inicializar modelo final
        final_model = TextModelForBinaryClassification(MODEL_NAME).to(DEVICE)


        # Optimizador con tasas de aprendizaje diferentes
        optimizer = optim.AdamW([
            {'params': final_model.transformer.parameters(), 'lr': best_params['learning_rate'] / 10},
        ...
        # Crear dataset con todos los datos de entrenamiento
        test_dataset = TextClassificationDataset(audio_paths_test, ground_truth, ids_test, processor)
